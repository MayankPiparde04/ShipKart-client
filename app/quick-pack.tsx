import { Ionicons } from "@expo/vector-icons";
import { router } from "expo-router";
import { StatusBar } from "expo-status-bar";
import React, { useState } from "react";
import {
  ActivityIndicator,
  Alert,
  KeyboardAvoidingView,
  Platform,
  ScrollView,
  Text,
  TextInput,
  TouchableOpacity,
  View,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";

// Import axios instance directly
import api from "@/services/api";

interface PackItem {
  id: string;
  name: string;
  length: string;
  breadth: string;
  height: string;
  weight: string;
  quantity: string;
}

export default function QuickPack() {
  const [items, setItems] = useState<PackItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<any>(null);

  // Form state
  const [showForm, setShowForm] = useState(false);
  const [newItem, setNewItem] = useState({
    name: "",
    length: "",
    breadth: "",
    height: "",
    weight: "",
    quantity: "1",
  });

  const addItem = () => {
    if (
      !newItem.name ||
      !newItem.length ||
      !newItem.breadth ||
      !newItem.height
    ) {
      Alert.alert("Error", "Please fill in Name and Dimensions (L, B, H)");
      return;
    }

    const item: PackItem = {
      id: Date.now().toString(),
      name: newItem.name,
      length: newItem.length,
      breadth: newItem.breadth,
      height: newItem.height,
      weight: newItem.weight || "0",
      quantity: newItem.quantity || "1",
    };

    setItems([...items, item]);
    setNewItem({
      name: "",
      length: "",
      breadth: "",
      height: "",
      weight: "",
      quantity: "1",
    });
    setShowForm(false);
  };

  const removeItem = (id: string) => {
    setItems(items.filter((item) => item.id !== id));
  };

  const calculatePacking = async () => {
    if (items.length === 0) {
      Alert.alert("Error", "Please add at least one item to pack");
      return;
    }

    setLoading(true);
    setResult(null);

    try {
      // Transform items for API
      const products = items.map((item) => ({
        name: item.name,
        dimensions: {
          length: parseFloat(item.length),
          breadth: parseFloat(item.breadth),
          height: parseFloat(item.height),
        },
        weight: parseFloat(item.weight),
        quantity: parseInt(item.quantity),
      }));

      // Call API using the centralized axios instance
      const response = await api.post("/enhanced-packing", { products });

      if (response.data && response.data.success) {
        setResult(response.data.data);
      } else {
        Alert.alert(
          "Error",
          response.data?.message || "Packing calculation failed",
        );
      }
    } catch (error: any) {
      console.error("Packing error:", error);
      Alert.alert(
        "Error",
        error.response?.data?.message ||
          error.message ||
          "Failed to calculate packing",
      );
    } finally {
      setLoading(false);
    }
  };

  const confirmPacking = async () => {
    Alert.alert(
      "Confirm Packing",
      "This will deduct the used cartons from your inventory. Continue?",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Confirm",
          onPress: async () => {
            setLoading(true);
            try {
              const { packingResults } = result;
              // Group by box name to minimize API calls
              const boxUsage: Record<string, number> = {};

              if (packingResults) {
                packingResults.forEach((carton: any) => {
                  const name = carton.cartonDetails.name;
                  boxUsage[name] = (boxUsage[name] || 0) + 1;
                });

                // Call removebox for each type
                for (const [boxName, qty] of Object.entries(boxUsage)) {
                  await api.post("/removebox", {
                    boxName: boxName,
                    quantity: qty,
                  });
                }
              }

              Alert.alert("Success", "Inventory updated successfully!", [
                {
                  text: "OK",
                  onPress: () => {
                    setResult(null);
                    setItems([]);
                    router.back();
                  },
                },
              ]);
            } catch (error: any) {
              console.error("Inventory update failed:", error);
              Alert.alert(
                "Error",
                "Failed to update inventory. Some items may not have been deducted.",
              );
            } finally {
              setLoading(false);
            }
          },
        },
      ],
    );
  };

  const renderResult = () => {
    if (!result) return null;

    const { summary, packingResults } = result;

    return (
      <View className="flex-1">
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-2xl font-bold text-gray-900 dark:text-white">
            Results
          </Text>
          <TouchableOpacity
            onPress={() => setResult(null)}
            className="bg-gray-200 dark:bg-gray-700 px-3 py-1 rounded-full"
          >
            <Text className="text-gray-700 dark:text-gray-300">Close</Text>
          </TouchableOpacity>
        </View>

        <View className="flex-row gap-3 mb-6">
          <View className="bg-green-100 dark:bg-green-900/30 p-4 rounded-xl flex-1 items-center">
            <Text className="text-3xl font-bold text-green-700 dark:text-green-400">
              {summary.totalCartonsUsed}
            </Text>
            <Text className="text-sm text-green-800 dark:text-green-300">
              Cartons
            </Text>
          </View>
          <View className="bg-blue-100 dark:bg-blue-900/30 p-4 rounded-xl flex-1 items-center">
            <Text className="text-3xl font-bold text-blue-700 dark:text-blue-400">
              {summary.packingRate}%
            </Text>
            <Text className="text-sm text-blue-800 dark:text-blue-300">
              Efficiency
            </Text>
          </View>
        </View>

        <Text className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-3">
          Carton Breakdown
        </Text>

        {packingResults.map((carton: any, index: number) => (
          <View
            key={index}
            className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 mb-3"
          >
            <View className="flex-row justify-between mb-2">
              <Text className="font-bold text-gray-900 dark:text-white text-lg">
                Carton #{index + 1}
              </Text>
              <Text className="text-gray-500 dark:text-gray-400">
                {carton.cartonDetails.name}
              </Text>
            </View>
            <Text className="text-gray-600 dark:text-gray-300 mb-2">
              {carton.cartonDetails.dimensions.length}x
              {carton.cartonDetails.dimensions.breadth}x
              {carton.cartonDetails.dimensions.height} cm
            </Text>

            <View className="h-px bg-gray-200 dark:bg-gray-700 my-2" />

            <Text className="font-medium text-gray-700 dark:text-gray-300 mb-1">
              Items ({carton.itemsPacked}):
            </Text>
            <Text className="text-gray-500 dark:text-gray-400 text-xs">
              See detailed analysis for arrangement.
            </Text>

            <View className="flex-row mt-3 gap-2">
              <View className="bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded">
                <Text className="text-xs text-gray-600 dark:text-gray-300">
                  Vol Eff: {carton.efficiency.volumeEfficiency}%
                </Text>
              </View>
              <View className="bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded">
                <Text className="text-xs text-gray-600 dark:text-gray-300">
                  Wt Util: {carton.efficiency.weightUtilization}%
                </Text>
              </View>
            </View>
          </View>
        ))}

        <TouchableOpacity
          className="bg-green-600 dark:bg-green-700 p-4 rounded-xl mt-4 flex-row justify-center items-center"
          onPress={confirmPacking}
        >
          <Text className="text-white font-bold text-lg">
            Confirm & Update Inventory
          </Text>
        </TouchableOpacity>
      </View>
    );
  };

  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-gray-950">
      <StatusBar style="auto" />
      <KeyboardAvoidingView
        behavior={Platform.OS === "ios" ? "padding" : "height"}
        className="flex-1"
      >
        <View className="px-4 py-4 border-b border-gray-200 dark:border-gray-800 flex-row items-center">
          <TouchableOpacity onPress={() => router.back()} className="mr-4">
            <Ionicons name="arrow-back" size={24} color="#6b7280" />
          </TouchableOpacity>
          <Text className="text-xl font-bold text-gray-900 dark:text-white">
            Quick Pack
          </Text>
        </View>

        <ScrollView
          className="flex-1 px-4 py-4"
          contentContainerStyle={{ paddingBottom: 100 }}
        >
          {result ? (
            renderResult()
          ) : (
            <>
              <Text className="text-gray-500 dark:text-gray-400 mb-4">
                Add items to your packing list below.
              </Text>

              {items.map((item) => (
                <View
                  key={item.id}
                  className="bg-gray-50 dark:bg-gray-900 p-4 rounded-xl mb-3 flex-row justify-between items-center border border-gray-200 dark:border-gray-800"
                >
                  <View className="flex-1">
                    <Text className="font-bold text-gray-900 dark:text-white text-lg">
                      {item.name}
                    </Text>
                    <Text className="text-gray-500 dark:text-gray-400 text-sm">
                      {item.length}x{item.breadth}x{item.height} cm •{" "}
                      {item.weight}g
                    </Text>
                  </View>
                  <View className="items-end">
                    <Text className="font-bold text-blue-600 mb-1">
                      x{item.quantity}
                    </Text>
                    <TouchableOpacity
                      onPress={() => removeItem(item.id)}
                      className="bg-red-100 p-2 rounded-full"
                    >
                      <Ionicons
                        name="trash-outline"
                        size={16}
                        color="#ef4444"
                      />
                    </TouchableOpacity>
                  </View>
                </View>
              ))}

              {items.length === 0 && (
                <View className="items-center justify-center py-10 opacity-50">
                  <Ionicons name="cube-outline" size={64} color="#9ca3af" />
                  <Text className="text-gray-400 mt-2">No items added yet</Text>
                </View>
              )}
            </>
          )}
        </ScrollView>

        {/* Bottom Action Bar */}
        {!result && (
          <View className="p-4 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
            <View className="flex-row gap-3">
              <TouchableOpacity
                onPress={() => setShowForm(true)}
                className="flex-1 bg-gray-200 dark:bg-gray-800 py-4 rounded-xl"
              >
                <Text className="text-center font-bold text-gray-900 dark:text-white">
                  Add Item
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={calculatePacking}
                className={`flex-1 bg-blue-600 py-4 rounded-xl ${
                  items.length === 0 ? "opacity-50" : ""
                }`}
                disabled={items.length === 0 || loading}
              >
                {loading ? (
                  <ActivityIndicator color="white" />
                ) : (
                  <Text className="text-center font-bold text-white">
                    Pack Now
                  </Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
        )}

        {/* Add Item Modal/Form Sheet */}
        {showForm && (
          <View className="absolute inset-x-0 bottom-0 bg-white dark:bg-gray-900 rounded-t-3xl shadow-2xl p-6 h-[70%] z-50">
            <View className="flex-row justify-between items-center mb-6">
              <Text className="text-xl font-bold text-gray-900 dark:text-white">
                Add Item
              </Text>
              <TouchableOpacity onPress={() => setShowForm(false)}>
                <Ionicons name="close" size={24} color="#6b7280" />
              </TouchableOpacity>
            </View>

            <ScrollView className="space-y-4">
              <View>
                <Text className="text-gray-500 mb-1">Name</Text>
                <TextInput
                  value={newItem.name}
                  onChangeText={(t) => setNewItem({ ...newItem, name: t })}
                  className="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl text-gray-900 dark:text-white"
                  placeholder="e.g. Headphones"
                  placeholderTextColor="#9ca3af"
                />
              </View>

              <View className="flex-row gap-3">
                <View className="flex-1">
                  <Text className="text-gray-500 mb-1">Length (cm)</Text>
                  <TextInput
                    value={newItem.length}
                    onChangeText={(t) => setNewItem({ ...newItem, length: t })}
                    className="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl text-gray-900 dark:text-white"
                    placeholder="0"
                    keyboardType="numeric"
                    placeholderTextColor="#9ca3af"
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-gray-500 mb-1">Breadth (cm)</Text>
                  <TextInput
                    value={newItem.breadth}
                    onChangeText={(t) => setNewItem({ ...newItem, breadth: t })}
                    className="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl text-gray-900 dark:text-white"
                    placeholder="0"
                    keyboardType="numeric"
                    placeholderTextColor="#9ca3af"
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-gray-500 mb-1">Height (cm)</Text>
                  <TextInput
                    value={newItem.height}
                    onChangeText={(t) => setNewItem({ ...newItem, height: t })}
                    className="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl text-gray-900 dark:text-white"
                    placeholder="0"
                    keyboardType="numeric"
                    placeholderTextColor="#9ca3af"
                  />
                </View>
              </View>

              <View className="flex-row gap-3">
                <View className="flex-1">
                  <Text className="text-gray-500 mb-1">Weight (g)</Text>
                  <TextInput
                    value={newItem.weight}
                    onChangeText={(t) => setNewItem({ ...newItem, weight: t })}
                    className="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl text-gray-900 dark:text-white"
                    placeholder="0"
                    keyboardType="numeric"
                    placeholderTextColor="#9ca3af"
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-gray-500 mb-1">Quantity</Text>
                  <TextInput
                    value={newItem.quantity}
                    onChangeText={(t) =>
                      setNewItem({ ...newItem, quantity: t })
                    }
                    className="bg-gray-100 dark:bg-gray-800 p-4 rounded-xl text-gray-900 dark:text-white"
                    placeholder="1"
                    keyboardType="numeric"
                    placeholderTextColor="#9ca3af"
                  />
                </View>
              </View>

              <TouchableOpacity
                onPress={addItem}
                className="bg-blue-600 py-4 rounded-xl mt-4"
              >
                <Text className="text-center font-bold text-white">
                  Add to List
                </Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        )}

        {/* Overlay for form */}
        {showForm && (
          <TouchableOpacity
            activeOpacity={1}
            onPress={() => setShowForm(false)}
            className="absolute inset-0 bg-black/50 z-40"
          />
        )}
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
}
